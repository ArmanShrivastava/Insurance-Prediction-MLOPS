...
for filepath in list_of_files:
    file_path  = Path(filepath)  #convert string to path object
    filedir , file_name = os.path.split(file_path) #split the path into directory and file name
    if filedir != "":
    ...
    if (not os.path.exists(file_path)) or (os.path.getsize(file_path) == 0):
        ...
        print(f"file is already exists : {file_path}")
